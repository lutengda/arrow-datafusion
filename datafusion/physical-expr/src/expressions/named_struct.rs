...
use arrow::{
    ...
    record_batch::RecordBatch, array::StructArray,
};
...
impl PhysicalExpr for NamedStruct {
    ...
    fn evaluate(&self, batch: &RecordBatch) -> Result<ColumnarValue> {
        let data = self.exprs
            .iter()
            .map(|(name, expr)| {
                Ok((name.as_str(), expr.evaluate(batch)?.into_array(1)))
            })
            .collect::<Result<Vec<_>>>()?;
        ...
        down_cast_any_ref(other)
            ...
            .map(|x| {
                self.exprs.len() == x.exprs.len()
                    && self.exprs.iter().zip(x.exprs.iter()).all(|(a, b)| {
                        a.0 == b.0 && a.1.eq(&b.1)
                    })
            })
            .unwrap_or(false)
